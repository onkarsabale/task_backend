...
export const createTask = async (data: CreateTaskDto, user: IUser) => {
    ...
    if (data.assignedTo && data.assignedTo.includes('@')) {
        ...
        if (!user) throw new AppError(`User with email ${data.assignedTo} not found`, 404);
        data.assignedTo = (user._id as unknown) as string;
    }

    const task = await taskRepo.createTask(data, (user._id as unknown) as string);
    const populatedTask = await taskRepo.findTaskById((task._id as unknown) as string);
    ...
    if (populatedTask?.assignedTo) {
        const assigneeId = (populatedTask.assignedTo as any)._id || populatedTask.assignedTo;
...
export const getTasks = async (user: IUser, filter: any = {}) => {
    // Visibility Rules
    // ADMIN: All
    ...
    if (filter.assignedTo && filter.assignedTo.includes('@')) {
        ...
        query.assignedTo = (assignedUser._id as unknown) as string;
...
export const getTaskById = async (id: string, user: IUser) => {
...
export const updateTask = async (id: string, data: UpdateTaskDto, user: IUser) => {
    ...
    const io = getIO();
    const assigneeId = task.assignedTo ? ((task.assignedTo as any)._id || task.assignedTo) : null;
    const creatorId = task.createdBy ? ((task.createdBy as any)._id || task.createdBy) : null;

    // 1. Task Updated -> Admins, Managers, Assignee, Creator
    io.to('room:admins').to('room:managers').emit('task:updated', task);
    if (assigneeId) io.to(`user:${assigneeId}`).emit('task:updated', task);
    if (creatorId && String(creatorId) !== String(assigneeId)) io.to(`user:${creatorId}`).emit('task:updated', task);
...
export const deleteTask = async (id: string, user: IUser) => {
    // Permission handled by Controller based on Project Role
    ...
    if (assigneeId) io.to(`user:${assigneeId}`).emit('task:deleted', id);
    if (creatorId && String(creatorId) !== String(assigneeId)) io.to(`user:${creatorId}`).emit('task:deleted', id);
